...
def compute_results(input_,field):   
	...
	if field == "molecule":
		...
		for item in ref:
			if isinstance(item, str):
				splitted = re.split(r'\s*(?:/|\\|(?<=\s)e(?=\s))\s*', item)
				tokenized_refs.extend(splitted)
		ref = [tok for tok in tokenized_refs if tok]
	gen=input_[1]
	...
	try:
		...
		gen_list=[]
	ref = [x for x in ref if not is_nan_or_empty(x)]
	gen_list = [x for x in gen_list if not is_nan_or_empty(x)]
	if not ref and not gen_list:
		f1score = 1.0
	else:
		f1score = calculate_list_f1(gen_list, ref)
	print("ref:", ref, "gen_list:", gen_list, "| f1score:", f1score)
